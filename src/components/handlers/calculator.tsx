...
import { type CalcDigitPayload, type CalcOperatorPayload } from '@events/handles/calculator'
import { Events } from '@types'
import { calculatorStore } from '@/stores/calculator'
import { add, divide, multiply, subtract, type CalculateArgs } from '@/utils/calculate'
...
function CalculatorHandler() {
  function onDigit(event: CustomEvent<CalcDigitPayload>) {
    ...
    if (state.overwrite) {
      calculatorStore.getState().set({ currentOperand: value, overwrite: false })
      return
    }
    ...
    calculatorStore.getState().set({ currentOperand: `${state.currentOperand}${value}` })
  }
  ...
  function onDecimal() {
    ...
    calculatorStore.getState().set({ currentOperand: `${state.currentOperand}.` })
  }
  ...
  function onEquals() {
    ...
    if (!state.operator || !state.previousOperand || state.currentOperand === 'Error') return
  ...
  function onDelete() {
    ...
    calculatorStore.getState().set({ currentOperand: state.currentOperand.slice(0, -1) })
  }
